...
def makeTopPlaylist(time_range):
    ...
    playlistID = sp.current_user_playlists(limit=1)['items'][0]['id']
    allSongs = list()
    for i in range(2):
        if i == 0:
            try:    
...
@app.route('/data')
def data():
    oldArtistRange = 'medium_term'
    oldArtistLimit = 10
    oldSongRange = 'medium_term'
    oldSongLimit = 10

    artistRange = request.args.get('artistRange', oldArtistRange)
    artistLimit = int(request.args.get('artistLimit', oldArtistLimit))
    songRange = request.args.get('songRange', oldSongRange)
    songLimit = int(request.args.get('songLimit', oldSongLimit))

    if artistRange != oldArtistRange:
        artistRange = artistRange
    else:
        artistRange = oldArtistRange

    if artistLimit != oldArtistLimit:
        artistLimit = artistLimit
    else:
        artistLimit = oldArtistLimit

    if songRange != oldSongRange:
        songRange = songRange
    else:
        songRange = oldSongRange

    if songLimit != oldSongLimit:
        songLimit = songLimit
    else:
        songLimit = oldSongLimit
